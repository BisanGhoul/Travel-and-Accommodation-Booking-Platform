...
console.log("Static icons dir:", path.join(__dirname, "public/icons"));
...
app.get("/api/home/search", async (req, res) => {
  ...
  if (adults) {
    filteredResults = filteredResults.filter(
      (room) => room.numberOfAdults >= Number(adults)
    );
  }

  if (children) {
    filteredResults = filteredResults.filter(
      (room) => room.numberOfChildren >= Number(children)
    );
  }

  if (numberOfRooms) {
    filteredResults = filteredResults.filter(
      (room) => room.numberOfRooms >= Number(numberOfRooms)
    );
  }
  ...
  if (checkInDate && checkOutDate) {
    ...
    filteredResults = filteredResults.filter((hotel) => {
      const hotelUnavailableStart = new Date(hotel.checkInDate);
      const hotelUnavailableEnd = new Date(hotel.checkOutDate);

      // Only include hotels where desired dates do NOT overlap with unavailable range
      const isAvailable =
        userCheckOut <= hotelUnavailableStart ||
        userCheckIn >= hotelUnavailableEnd;
...
app.put("/api/rooms/:id", (req, res) => {
  ...
  const updatedRoom = { ...req.body, roomId };
...
app.post("/api/rooms", (req, res) => {
  ...
  const lastId = data.length > 0 ? data[data.length - 1].roomId : 0;
  const newRoom = {
    roomId: lastId + 1,
    ...req.body,
  };
